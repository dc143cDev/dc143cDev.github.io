...
<figure>
  <img src="/assets/images/post-250502-001.jpeg" alt="" class="screenshot">
  <figcaption>기타와 생활 소음 사운드 FFT 스펙트럼 비교</figcaption>
</figure>

위 두 스크린샷은 제가 만든 테스트 앱에서 기타 사운드와 생활 소음의 FFT 스펙트럼 변화를 비교한 것입니다. 

(왼쪽) 생활 소음은 불규칙한 봉우리들이 마치 **산맥과도 같은 형태**를 띄지만,
(오른쪽) 기타를 연주할 때는 **규칙적인 간격으로 높게 솟은 봉우리들**이 나타납니다.
...
기타 유효 대역 내에서, 진폭을 내림차순으로 정렬한 후, 최대 진폭의 60% 이상인 피크들을 최대 5개까지 **기본음 후보**로 간주합니다.

기본음이란, 배음의 원리를 설명드릴 때 말씀드렸던 **배음의 원음**으로, 감지된 연주 단위가 **단일 타현**인지, **다중현 타음**인지 구분할 때 가장 중요하게 사용되는 데이터입니다.
...
가산 요소는,
① 기본음 후보가 상위 피크에 포함돼 있는지(기본음 보너스), 
② 피크 간 빈도 비율이 정수배/분수배에 근접한지(배음 관계), 
③ 피크들이 일반적인 기타 범위(약 80–1320 Hz)에 분포하는지(범위 점수), 
④ 외부로 전달된 currentPitch와 직접/옥타브 일치하는 피크가 있는지(피치 일치)입니다. 
...
이로써 위의 로직들을 거쳐, 
최종적으로 환산된 점수들을 종합적으로 평가한 값인 **smoothedConfidence**를 기반으로 기타 연주 판정을 내립니다.


### 한계
이 로직의 한계점과 주의사항
폴리포니/스트럼 상황의 모호성: 동시에 많은 피크가 뜨면 기본음·배음 구조가 흐려져 topPeaksScore와 harmonicScore가 불안정해질 수 있음. 분포 점수(Fallback)에 과의존할 수 있음.
하이게인/이펙팅의 스펙트럼 왜곡: 디스토션, 포화, 강한 앰프/캡 EQ는 배음 감쇠 패턴을 비정상적으로 만들고 고역 노이즈를 과다 발생시켜 오판정을 유발할 수 있음.
비기타 음원의 유사 배음: 일부 신시사이저, 피아노, 보컬의 포먼트가 정수배 관계를 흉내 내면 topPeaksScore가 과대평가될 수 있음.
피치 없는 어택/퍼커시브 노이즈: 피크는 있으나 조음이 불명확한 타격음에서는 기본음 후보가 없어 Fallback 위주로 보수적 판단이 이뤄져 민감도가 떨어질 수 있음.
장비·환경 의존성: 마이크, 샘플링 레이트/FFT 크기(고정 가정: 44.1k/4096), 녹음 레벨, 룸 톤 차이에 따라 임계·분포 기준이 최적이 아닐 수 있음.
장르·주법 편향: 팜뮤트/헤비 톤, 이코라이징이 강한 톤은 중·저역 분포가 치우쳐 분포 점수가 흔들릴 수 있음.
실시간 제약과 평활화 지연: EMA로 안정성은 좋아지지만 순간 변화에 대한 반응은 약간 느려질 수 있음(임계 부근에서 on/off 깜빡임 감소의 대가).
단일 프레임 의사결정의 한계: 국지 프레임 판단이므로 상위 맥락(연속 프레임의 의도/포지션 흐름)을 보지 못함. 오탐 시 연쇄 보정이 어려움.
