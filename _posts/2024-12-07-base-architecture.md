...
제가 소개하는 베이스 아키텍처는 현업에서 프로젝트를 진행하며 저와 팀원 분들의 작업 환경에 맞추어 설계되었기에, 당연하게도 모든 프로젝트에 완벽하게 기능하는 구조는 아닙니다.

다만 다음과 같은 목적을 위해 설계되었기에, 저희와 목적이 같은 분들에게는 참고가 될 수 있습니다.

- 10명 이하의 팀원으로 구성되어, 10개에서 30개 이하의 페이지를 구성하는 소, 중규모 프로젝트 대상
- 디자이너와 협업하며, 대다수의 페이지가 특정 스타일 가이드를 따르는 프로젝트
- 비즈니스 로직과 데이터베이스 통신이 필요하나, 앱에 저장되는 영역과 View Model 영역이 명확하게 구분되는 프로젝트
- GetX 생태계가 익숙한 팀원들이 빠르게 개발할 수 있는 프로젝트
